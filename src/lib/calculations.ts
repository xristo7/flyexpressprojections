export type Drivers = {
  ...
  parcel_fee: number
  pickup_adoption: number
  pickup_fee: number
  lastmile_adoption: number
  lastmile_fee: number
  ...
  pickup_central: number
  lastmile_central: number
...
export const DRIVER_META: DriverMeta[] = [
  ...
  { key: 'parcels_per_day', label: 'Parcels per vehicle / day', unit: 'parcels', purpose: 'Daily parcel volume per vehicle (base for transport + pickup + last-mile)' },
  { key: 'parcel_fee', label: 'Average parcel fee (stage-to-stage)', unit: 'UGX', purpose: 'Stage-to-stage transport fee per parcel', step: 500 },
  { key: 'pickup_adoption', label: 'Sender pickup adoption', unit: 'ratio', purpose: 'Share of parcels that use sender pickup (independent of last-mile)', step: 0.01 },
  { key: 'pickup_fee', label: 'Average sender pickup fee', unit: 'UGX', purpose: 'Average fee charged for sender pickup', step: 500 },
  { key: 'lastmile_adoption', label: 'Last-mile delivery adoption', unit: 'ratio', purpose: 'Share of parcels that use last-mile delivery (independent of pickup)', step: 0.01 },
  { key: 'lastmile_fee', label: 'Average last-mile fee', unit: 'UGX', purpose: 'Average fee charged for last-mile delivery', step: 500 },
  ...
  { key: 'parcel_central', label: 'Parcel transport central share', unit: 'ratio', purpose: 'Share of parcel transport gross retained centrally', step: 0.01 },
  { key: 'pickup_central', label: 'Sender pickup central share', unit: 'ratio', purpose: 'Share of sender pickup gross retained centrally', step: 0.01 },
  { key: 'lastmile_central', label: 'Last-mile central share', unit: 'ratio', purpose: 'Share of last-mile delivery gross retained centrally', step: 0.01 },
...
export const DEFAULT_STREAM_ACTIVE: Record<StreamId, boolean> = {
  ...
  parcel_transport: true,
  sender_pickup: true,
  lastmile_delivery: true,
  luggage: true,
  return_ticket: true,
  advertising: true,
}
...
export function computeProjections(
  ...
  // Parcel transport (stage-to-stage)
  ...
  // Sender pickup (independent of last-mile)
  const pickupDaily = d.vehicles * d.parcels_per_day * d.pickup_adoption * d.pickup_fee
  const pickupMonthly = pickupDaily * d.days
  const pickupMonthlyCentral = pickupMonthly * d.pickup_central

  // Last-mile delivery (independent of pickup)
  const lastmileDaily = d.vehicles * d.parcels_per_day * d.lastmile_adoption * d.lastmile_fee
  const lastmileMonthly = lastmileDaily * d.days
  const lastmileMonthlyCentral = lastmileMonthly * d.lastmile_central
  ...
  const streams: StreamResult[] = [
    ...
    mk('parcel_transport', 'Parcel transport', parcelDaily, parcelMonthly, parcelMonthlyCentral),
    mk('sender_pickup', 'Sender pickup', pickupDaily, pickupMonthly, pickupMonthlyCentral),
    mk('lastmile_delivery', 'Last-mile delivery', lastmileDaily, lastmileMonthly, lastmileMonthlyCentral),
